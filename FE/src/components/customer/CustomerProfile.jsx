...
function Tl({ children, className = '' }) {
  return <span className={`font-semibold text-slate-900 ${className}`}>{children}</span>;
}
...
export default function CustomerProfile({ apiBase, token }) {
  ...
  const TIER_MAP = {
    ...
    diamond: { label: 'Kim cương', icon: Sparkle, color: 'text-blue-500 bg-blue-50' },
  };
          ...
          <div className="w-full max-w-sm space-y-4">
            <div>
              ...
              <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
                className={`w-full rounded-lg border ${errors.name ? 'border-red-300 ring-2 ring-red-100' : 'border-slate-200'} bg-white px-3 py-2 text-sm text-slate-800 placeholder:text-slate-400 focus:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-100 transition-colors`} />
            ...
            <div>
              ...
              <input value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })}
                className={`w-full rounded-lg border ${errors.phone ? 'border-red-300 ring-2 ring-red-100' : 'border-slate-200'} bg-white px-3 py-2 text-sm text-slate-800 placeholder:text-slate-400 focus:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-100 transition-colors`} />
            ...
            <div>
              ...
              <input type="date" value={form.dateOfBirth} onChange={(e) => setForm({ ...form, dateOfBirth: e.target.value })}
                className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-800 focus:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-100 transition-colors" />
            </div>
            <div className="flex gap-2 pt-1">
              <button onClick={handleSave} disabled={saving}
                className="flex flex-1 items-center justify-center gap-1.5 rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-blue-700 disabled:opacity-50 transition-all">
                {saving ? <GenericSpinner size={16} /> : <Check size={16} weight="bold" />}
                Lưu
              </button>
      ...
      <SectionCard title="Thông tin cá nhân" icon={User}>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div className="flex items-center gap-3 rounded-xl bg-slate-50 p-3.5">
            ...
            <div>
              <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Điện thoại</p>
          ...
          <div className="flex items-center gap-3 rounded-xl bg-slate-50 p-3.5">
            ...
            <div>
              <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Ngày sinh</p>
          ...
          <div className="flex items-center gap-3 rounded-xl bg-slate-50 p-3.5">
            ...
            <div>
              <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Vai trò</p>
              <p className="text-sm font-medium text-slate-800 capitalize">{profile?.role || '—'}</p>
            </div>
          </div>
        </div>
      </SectionCard>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-slate-500">
              <Tl>{profile?.loyaltyPoints ?? 0}</Tl> điểm tích lũy
            </p>
            <div className="mt-2 h-2 w-full max-w-[200px] overflow-hidden rounded-full bg-slate-100">
              <div className="h-full rounded-full bg-blue-500 transition-all duration-500"
                style={{ width: `${Math.min((profile?.loyaltyPoints ?? 0) / 1000 * 100, 100)}%` }} />
            </div>
          </div>
        </div>
      </SectionCard>

      <SectionCard title="Xe của tôi"
        icon={Car}
        action={
        ...
        {vehicles.length === 0 ? (
          ...
          <div className="space-y-3">
            {vehicles.map((v) => (
              <div key={v._id} className="flex items-center gap-4 rounded-xl border border-slate-100 bg-slate-50/50 p-4">
                <VehicleIcon type={v.vehicleType} />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <p className="text-sm font-semibold text-slate-800">{v.licensePlate}</p>
                    {v.isDefault && (
                      <span className="rounded-full bg-blue-50 px-2 py-0.5 text-[10px] font-semibold text-blue-600">Mặc định</span>
                    )}
                  </div>
                  <p className="text-xs text-slate-500">
                    {v.brand}{v.model ? ` ${v.model}` : ''} &middot; {VEHICLE_TYPE_LABELS[v.vehicleType] || v.vehicleType}
