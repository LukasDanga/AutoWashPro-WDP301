...
function CustomerDetailModal({ customer, onClose, filter }) {
  ...
  useEffect(() => {
    if (user?._id) {
      setLoadingBookings(true);
      const periodQ = filter !== 'all' ? `?period=${filter}` : '';
      api(`/bookings/user/${user._id}${periodQ}`)
        .then(r => r.json())
    ...
    <div className="fixed inset-0 z-[9999] bg-black/40 backdrop-blur-sm flex items-center justify-center p-4"
      ...
      <div className="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-xl" onClick={e => e.stopPropagation()}>
        ...
        {user ? (
          <>
            <div className="flex gap-1 bg-slate-100 rounded-xl p-1 mx-6 mb-5">
              {['info', 'benefits', 'bookings', 'vehicles', 'wallet'].map(t => (
                <button key={t} onClick={() => setTab(t)}
                  className={`flex-1 px-3 py-2 rounded-lg text-xs font-medium transition-all ${
                    tab === t ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                  }`}>
                  {t === 'info' ? 'Thông tin' : t === 'benefits' ? 'Ưu đãi hạng' : t === 'bookings' ? 'Lịch sử đặt' : t === 'vehicles' ? 'Xe' : 'Ví'}
                </button>
              ))}
            </div>
...
export default function ManagerRevenue() {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      {customerDetail && <CustomerDetailModal customer={customerDetail} onClose={() => setCustomerDetail(null)} filter={filter} />}
    </div>
  );
}
