...
export default function ManagerCheckInConfirmModal({ booking, onClose, onConfirmed }) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(err.message || 'Không thể xác nhận check-in');
      }
  ...
  const handleReject = async () => {
    ...
    try {
      await fetch(`${apiBase}/bookings/${booking._id}/reject-checkin`, {
        ...
        body: JSON.stringify({ reason: 'Quản lý từ chối / hủy yêu cầu check-in' }),
      }).catch(() => {});
    } finally {
      setBusy(false);
      onClose();
  ...
  const customerName = booking.userId?.name || booking.userId?.fullName || booking.customerName || booking.customerSnapshot?.name || 'Khách hàng';
  ...
  const packageName = booking.packageId?.name || booking.packageName || 'Dịch vụ rửa xe';
  const packagePrice = booking.packagePrice ?? booking.packageId?.price ?? 0;
  const branchName = booking.branchId?.name || booking.branchName || 'Chi nhánh';
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="text-xl font-black text-slate-900">Yêu cầu Check-in từ Khách</h3>
            <p className="text-xs text-slate-500 font-medium">Mã đơn: #{booking._id?.slice(-8).toUpperCase()}</p>
          </div>
        </div>
        ...
        {!isToday ? (
          <div className="mb-5 p-4 bg-amber-50 border border-amber-200 rounded-2xl flex items-start gap-3">
            ...
            <div>
              <p className="text-xs font-bold text-amber-800">CẢNH BÁO: ĐƠN ĐẶT KHÔNG PHẢI HÔM NAY!</p>
              <p className="text-xs text-amber-700 mt-1 leading-relaxed">
                Lịch này khách đặt ngày <span className="font-extrabold">{formatDate(bDateStr)}</span> (Hôm nay là {formatDate(todayStr)}).
          ...
          <div className="mb-5 p-3.5 bg-emerald-50 border border-emerald-200 rounded-2xl flex items-center gap-2 text-emerald-800 text-xs font-bold">
            ...
            Lịch đặt đúng ngày hôm nay ({formatDate(todayStr)})
          </div>
        )}
