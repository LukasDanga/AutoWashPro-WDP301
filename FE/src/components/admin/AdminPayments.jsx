...
export default function AdminPayments({ showDelete = true, detailPath = '/admin/payments', urlSync = false } = {}) {
  ...
  function validateDeleteRange(from, to) {
    if (deleteAll) return '';
    if (!from || !to) return 'Vui lòng chọn cả ngày bắt đầu và ngày kết thúc';
    if (from > to) return 'Ngày bắt đầu không được lớn hơn ngày kết thúc';
    const today = new Date().toISOString().slice(0, 10);
    if (to > today) return 'Ngày kết thúc không được ở tương lai';
    return '';
  }
  ...
  async function deletePaymentRow(p) {
    if (!(await confirmDialog({ title: 'Xác nhận xóa giao dịch', message: `Bạn có chắc muốn xóa giao dịch ${p.transactionId || ''}? Hành động này không thể hoàn tác!`, danger: true }))) return;
    setDeleting(true);
    try {
      const res = await api(`/payments/${p._id}`, { method: 'DELETE' });
      if (!res.ok) { const e = await res.json(); throw new Error(e.message || 'Xóa thất bại'); }
      const result = await res.json();
      showToast(result.message || 'Đã xóa giao dịch', 'success');
      load();
    } catch (e) { showToast(e.message, 'error'); }
    finally { setDeleting(false); }
  }
                    ...
                    <tr key={p._id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-1">
                          <button onClick={() => handleOpenDetail(p)} title="Xem chi tiết"
                          ...
                          {showDelete && (
                            <button onClick={() => deletePaymentRow(p)} title="Xóa giao dịch"
