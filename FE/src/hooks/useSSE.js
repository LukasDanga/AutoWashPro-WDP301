...
const SYNC_EVENTS = ['slots_updated', 'vouchers_updated', 'my_bookings_updated', 'feedback_new', 'booking_new', 'my_vehicles_updated', 'config_updated', 'branch_sort_order_updated', 'rewards_updated', 'my_rewards_updated'];
...
class SocketManager {
  constructor() {
    ...
    this.syncDebounceMs = 800; // Tránh burst refetch dồn dập
  ...
  connect(token) {
    ...
    this.socket.on('connect', () => {
      ...
      // Chỉ fire 1 LẦN DUY NHẤT ở initial connect của phiên để tránh nhảy UI.
  ...
  fireSyncEvents() {
    // Debounce: gom nhiều gọi trong syncDebounceMs thành 1 lần gọi listener cuối cùng.
